...
class SetDrivingSide(_BaesADBShellCommadForPackage):
    ...
    def __init__(self, reporting: enna.core.reporting.interface.Interface, adb: enna.data_interfaces.adb.interface.Interface):
        ...
        :param reporting: enna.core.reporting.interface.Interface,
        :param adb: enna.data_interfaces.adb.interface.Interface"""
    ...
    def _action(self) -> bool:
        ...
        try:
            result = set_driving_side(adb=self._adb, side=side)
        except enna.data_interfaces.adb.exceptions.ADBException as error:
            self._reporting.add_report_message_ta_error(f"Failed to set driving side: {error}")
            return False

        if not result:
            self._reporting.add_report_message_system_error(f"Failed to change driving side to {side}")
            return False

        self._reporting.add_report_message_pass(f"Driving side successfully changed to {side}")
        return True
