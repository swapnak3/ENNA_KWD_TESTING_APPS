...
class CheckZoomMode(enna_kwd_testing.stimulations.base.keyword_stim_baseclass.KeywordStimulation):
	"""Class containing functionality to check zoom mode in navigation."""
	...
	def _action(self) -> bool:
		...
		2. Set zoom mode as defined in instance attribute "MODE: MANUAL or AUTO" key "mode".
		...
		if "AUTO" in str(zoom_mode_new):
			if wrapper_android_hmi.wait_for_layout_element(str_xpath__zoom_mode_auto, self.__android_hmi, max_time=10):
				MODULE_LOGGER.info(f" Zoom mode already correct = {zoom_mode_new} .")
				return True
		elif "MANUAL" in str(zoom_mode_new):
			if wrapper_android_hmi.wait_for_layout_element(str_xpath__zoom_mode_manual, self.__android_hmi, max_time=10):
				MODULE_LOGGER.info(f" Zoom mode already correct = {zoom_mode_new} .")
				return True
