...
class CheckSystemBluetoothState(enna_kwd_testing.stimulations.base.keyword_stim_baseclass.KeywordStimulation):
	"""Class containing functionality to check system bluetooth state in settings."""
	...
	def _action(self) -> bool:
		...
		2. Check system bluetooth state in settings
		...
		if bt_state_new == "ACTIVATED":
			...
			MODULE_LOGGER.error(
				f"Wrong parameter for STATE:'{bt_state_new}', should be 'ACTIVATED' or 'DEACTIVATED")
			return False
		...
		if (state := wrapper_android_hmi.wait_and_get_toggle_button_state(str_switch_control_button, self.__android_hmi, self._reporting, f"Could not get toggle button state for '{str_switch_control_button}' button. An error occurred.")) is None:
			return False
