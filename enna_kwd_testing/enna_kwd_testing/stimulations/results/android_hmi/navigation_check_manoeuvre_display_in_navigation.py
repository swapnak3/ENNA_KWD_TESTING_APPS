...
class CheckManoeuvreDisplayInNavigation(enna_kwd_testing.stimulations.base.keyword_stim_baseclass.KeywordStimulation):
	"""Class containing functionality to check manoeuvre display in navigation."""
	...
	def _action(self) -> bool:
		...
		2. Check 'manoeuvre display in navigation'
		...
		str__show_maneuver_xpath = self._xpaths.get_xpath("settings", "settingShowManeuver")
		str__show_maneuver_switch_xpath = self._xpaths.get_xpath("settings", "settingShowManeuver-switch")

		try:
			str__state = self.values["STATE"]
			str__state = "true" if str__state == "ACTIVATED" else "false"
			str__show_maneuver_switch_checked_xpath = f"{str__show_maneuver_switch_xpath}[@checked='{str__state}']"
		except KeyError as exception:
			MODULE_LOGGER.error(f"Value {exception} not found")
			return False

		if not wrapper_android_hmi.wait_and_scroll_in_list(str__show_maneuver_xpath, str__navi_settings_list_xpath, self.__android_hmi):
			self._reporting.add_report_message_ta_error(f"Could not scroll to element: {str__show_maneuver_xpath} in list {str__navi_settings_list_xpath}.")
			return False

		if not wrapper_android_hmi.wait_for_layout_element(str__show_maneuver_switch_checked_xpath, self.__android_hmi):
			self._reporting.add_report_message_system_error(f"The manoeuvre display in navigation is not set to {self.values['STATE']}")
			return False

		MODULE_LOGGER.info(f"The manoeuvre display in navigation is set to {self.values['STATE']}")
