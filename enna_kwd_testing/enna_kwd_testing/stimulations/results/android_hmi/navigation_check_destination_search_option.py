...
class CheckNaviDestinationSearchOption(enna_kwd_testing.stimulations.base.keyword_stim_baseclass.KeywordStimulation):
	"""Class containing functionality to check option search is set."""
	...
	def _action(self) -> bool:
		...
		1. Navigate to 'navigation.Search' screen
		2. Check if search option is set
		...
		try:
			str_search_option_to_check = self.values["OPTION"].lower()
		...
		str__search_button_set_to_xpath = self._xpaths.get_xpath("search", f"searchButton_set_{str_search_option_to_check}")

		if not wrapper_android_hmi.wait_for_layout_element(str__search_button_set_to_xpath, self.__android_hmi):
			self._reporting.add_report_message_info(f"Could set search option to: {str_search_option_to_check}.")
			return False

		MODULE_LOGGER.info(f"The search option is set to {str_search_option_to_check}")
