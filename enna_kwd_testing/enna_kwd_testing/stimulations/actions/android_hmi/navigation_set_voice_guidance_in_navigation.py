...
class SetVoiceGuidanceInNavigation(enna_kwd_testing.stimulations.base.keyword_stim_baseclass.KeywordStimulation):
	"""Class containing functionality to set voice guidance in navigation."""
	...
	def __init__(self, reporting: enna.core.reporting.interface.Interface, android_hmi: enna_st12.data_interfaces.android_hmi.interface.Interface, menu_navigation: enna_st12.utilities.menu_navigation.interface.Interface):
		...
		self.allowed_parameter_keys = ["LENGTH"]
	...
	def _action(self) -> bool:
		...
		2. Scroll to 'voice guidance' in list on screen
		3. Set voice guidance.
		...
		try:
			str__voice_guidance_mode = f"voice_guidance_mode_{self.values['LENGTH'].lower()}"
		...
		if not wrapper_android_hmi.go_to_screen(navigation.SETTINGS, self._reporting, self.__menu_navigation):
			self._reporting.add_report_message_ta_error("Could not go to screen: 'NAVI.APP -> Settings. Something went wrong while navigating to screen.")
			return False

		str__navi_settings_list_xpath = self._xpaths.get_xpath("settings", "main_list")
		str__navi_settings_voice_guidance_xpath = self._xpaths.get_xpath("settings", str__voice_guidance_mode)

		if not wrapper_android_hmi.wait_and_scroll_in_list(str__navi_settings_voice_guidance_xpath, str__navi_settings_list_xpath, self.__android_hmi):
			self._reporting.add_report_message_ta_error(f"Could not scroll to element: {str__navi_settings_voice_guidance_xpath} in list {str__navi_settings_list_xpath}.")
			return False

		if not wrapper_android_hmi.wait_and_click(str__navi_settings_voice_guidance_xpath, self.__android_hmi, self._reporting):
			self._reporting.add_report_message_ta_error(f"Could not click element: {str__navi_settings_voice_guidance_xpath}.")
			return False

		str__navi_settings_voice_guidance_checked_xpath = f"{str__navi_settings_voice_guidance_xpath}[@checked='true']"

		if not wrapper_android_hmi.wait_for_layout_element(str__navi_settings_voice_guidance_checked_xpath, self.__android_hmi):
			self._reporting.add_report_message_info(f"Voice guidance is not set to {self.values['LENGTH']}")
			return False

		MODULE_LOGGER.info(f"The voice guidance is set to '{self.values['LENGTH']}' ")
