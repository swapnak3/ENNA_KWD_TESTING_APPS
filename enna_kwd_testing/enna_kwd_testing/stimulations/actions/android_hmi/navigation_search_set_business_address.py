...
class SetBusinessAddress(enna_kwd_testing.stimulations.base.keyword_stim_baseclass.KeywordStimulation):
	"""Class containing functionality to set business address in navi.app.
	parameter ADDRESS: str -> default business address 'Carl-Benz-Ring 7, 85080 Gaimersheim'
	"""
	...
	def __init__(self, reporting, android_hmi: enna_st12.data_interfaces.android_hmi.interface.Interface, menu_navigation: enna_st12.utilities.menu_navigation.interface.Interface):
		...
		self.default_business_address = "Carl-Benz-Ring 7, 85080 Gaimersheim"

	def _action(self) -> bool:
		"""Execute action.

		set business address in navi.app.

		1. Navigate to 'navi -> search -> Heimatadresse' screen
		2. Enter business address 'Carl-Benz-Ring 7, 85080 Gaimersheim'
		...
		str_search_list_business_item_details = self.__xpathloader.get_xpath_app_screen_id(reporting=self._reporting, android_hmi=self.__android_hmi, element="searchListBusinessItemDetails")
		str_search_business_address_label = self.__xpathloader.get_xpath_app_screen_id(reporting=self._reporting, android_hmi=self.__android_hmi, element="searchBusinessAddressLabel")
		if wrapper_android_hmi.element_exists(str_search_list_business_item_details, android=self.__android_hmi, additional_message="list_search_list_result_item", max_time=5):
			if not wrapper_android_hmi.wait_and_long_click_variable_duration(str_search_business_address_label, self.__android_hmi, self._reporting, f"Could not find and click '{str_search_business_address_label}' in destination list.", 1.5, 15.0):
				return False
		...
		enna.core.time.sleep(2)
		if not wrapper_android_hmi.wait_and_click_in_list(str_search_business_address_label, str_recycler_view_list, self.__android_hmi, f"Could not find and click '{str_search_business_address_label}' in destination list."):
			return False

		# self._speller.enter_text(str_business_address, True)
		self.__android_hmi.enter_text(text=str_business_address, clear=True)  # enter text if keyboard is open
		...
		if not wrapper_android_hmi.wait_and_click_in_list(str_search_list_result_item_0, str_recycler_view_list, self.__android_hmi, f"Could not find and click '{str_search_business_address_label}' in destination list."):
			return False
		if not wrapper_android_hmi.wait_for_screen_id(navigation_contexts.SEARCH_DETAILS, android=self.__android_hmi, additional_message="wait for screen", max_time=30):
			return False
