...
class SetPreferAudiChargingService(enna_kwd_testing.stimulations.base.keyword_stim_baseclass.KeywordStimulation):
	"""Class containing functionality to set prefer audi charging service."""
	...
	def _action(self) -> bool:
		...
		2. Set 'Prefer Brand Charging Stations-switch'
		...
		str__prefer_brand_charging_settings_xpath = self._xpaths.get_xpath("settings", "prefer_brand_charging_station")
		...
		if not wrapper_android_hmi.wait_for_layout_element(str__prefer_brand_charging_settings_xpath, self.__android_hmi):
			self._reporting.add_report_message_info(f"The charging settings xpath {str__prefer_brand_charging_settings_xpath} is not shown")
			return False
		str__button_state = self.__android_hmi.layout.value.xpath(str__prefer_brand_charging_settings_switch_xpath)[0].attrib["checked"]

		if str__button_state != str__state:
			if not wrapper_android_hmi.wait_and_click(str__prefer_brand_charging_settings_xpath, self.__android_hmi, self._reporting):
				self._reporting.add_report_message_ta_error(f"Could not click element: {str__prefer_brand_charging_settings_xpath}.")
				return False
		...
		MODULE_LOGGER.info(f"The prefer audi charging setting is set to {self.values['STATE']}")
