...
class SetStatisticsInOlb(enna_kwd_testing.stimulations.base.keyword_stim_baseclass.KeywordStimulation):
	"""Class containing functionality to set statistics variant in olb.app under statistic_settings."""
	...
	def _action(self) -> bool:
		...
		set statistics variant in olb.app under statistic_settings.

		1. Navigate to 'olb.SETTINGS' screen
		2. Click on Button (STATISTIC: ["Current year", "Comparison to the same period...", "Comparison to the entire previous year"])
		...
		try:
			...
			str__set_statistic_button_xpath = ""
			if str__statistic_value in self.allowed_parameter_values:
				for i, j in enumerate(self.allowed_parameter_values):
					if str__statistic_value == j:
						str__set_statistic_button_xpath = self._xpaths.get_xpath("settings", self.olb_button_xpath_values[i])
				if str__set_statistic_button_xpath == "":
		...
		error_message = f"Could not press button '{str__statistic_value}' button. Xpath '{str__set_statistic_button_xpath}' not found. An error occurred."

		if not wrapper_android_hmi.wait_and_click(xpath_element=str__set_statistic_button_xpath, android=self.__android_hmi, reporting=self._reporting, additional_message="", max_time=15):
			self._reporting.add_report_message_system_error(error_message)
			return False
		if not wrapper_android_hmi.wait_for_layout_element(xpath=f"{str__set_statistic_button_xpath}[@checked='true']", android=self.__android_hmi, additional_message="", max_time=15):
			self._reporting.add_report_message_system_error(error_message)
			return False
